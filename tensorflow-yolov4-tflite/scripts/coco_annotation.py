...
flags.DEFINE_string('coco_data', './annotations_train_coco.pkl', 'path to coco data')
flags.DEFINE_string('classes', './data/classes/innov_plus_classes.names', 'path to classes file')
flags.DEFINE_string('coco_path', "../data/full_data", 'resize images to')
flags.DEFINE_string('image_path', "images/train", 'path to image val')
flags.DEFINE_string('anno_path_val', './data/dataset/train_labels.txt', 'path to classes file')
...
def convert_annotation(output, data, data_type = "val"):
    ...
    with open(output, 'a') as f:
        for image_path in image_paths:
            image_inds = '.'.join(image_path.split(".")[:-1])
